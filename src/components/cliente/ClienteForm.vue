<template>
  ...
  >
    <div class="field">
      ...
      <InputText id="nombre" v-model="formData.nombre" required="true" autofocus :class="{ 'p-invalid': submitted && !formData.nombre }" />
      <small class="p-error" v-if="submitted && !formData.nombre">El nombre es requerido.</small>
    </div>

    <div class="field">
      <label for="apellido">Apellido</label>
      <InputText id="apellido" v-model="formData.apellido" required="true" :class="{ 'p-invalid': submitted && !formData.apellido }" />
      <small class="p-error" v-if="submitted && !formData.apellido">El apellido es requerido.</small>
    </div>

    <div class="field">
      <label for="email">Email</label>
      <InputText id="email" v-model="formData.email" required="true" type="email" :class="{ 'p-invalid': submitted && !formData.email }" />
      <small class="p-error" v-if="submitted && !formData.email">El email es requerido.</small>
    </div>

    <div class="field">
      <label for="telefono">Teléfono</label>
      <InputText id="telefono" v-model="formData.telefono" required="true" :class="{ 'p-invalid': submitted && !formData.telefono }" />
      <small class="p-error" v-if="submitted && !formData.telefono">El teléfono es requerido.</small>
    </div>

    <template #footer>
      <Button label="Cancelar" icon="pi pi-times" outlined @click="$emit('close')" class="p-button-text" />
